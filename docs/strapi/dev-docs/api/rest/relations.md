...
shorthand" label="Shorthand syntax example">
...
longhand" label="Longhand syntax example">
...
The syntaxes described in this documentation are useful for one-to-many, many-to-many and many-ways relations.For one-to-one, many-to-one and one-way relations, the syntaxes are also supported but only the last relation will be used, so it's preferable to use a shorter format (e.g.: `{ data: { category: 2 } }`, see [REST API documentation](/dev-docs/api/rest#requests)).
:::
...
combined" label="Combined example">
