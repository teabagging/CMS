...
simple-token" label="Simple token">
...
environment-variable" label="Environment variable">
...
| [`releases.publish`](#releasespublish-) | Triggered when a Release is published (see [Releases](/user-docs/releases/introduction)).This event is only available with the  edition of Strapi and the  plan for Strapi Cloud. |
